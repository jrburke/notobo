  ...
  try {
    ...
    if (deps && deps.length) {
      deps.forEach(function(dep, i) {
        // Browserify dumps garbage in the require API by using numbers
    ...
    // If the file begins with #!/usr/bin/env node, then skip it,
    // it is not something meant to be run in the browser.
    if (fileContents && fileContents.indexOf('#!') === 0) {
      return returnValue(fileContents);
    }
